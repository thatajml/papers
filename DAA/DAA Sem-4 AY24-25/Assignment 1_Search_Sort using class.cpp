...
int Records :: partition(int low,int high){
	int i,j;
	long int piv = M[low].mobileno;
	MobileUser temp;
	i = low + 1;
	j = high;
	while(1)
	{
		while(M[i].mobileno >= piv && i < high)
			i++;
		while(M[j].mobileno <= piv && j > low)
			j--;
		if(i < j){
		    temp = M[i];
		    M[i] = M[j];
...
void Records :: non_rec_quick(int low,int high)
{
	int j = 0;
	push(low ,high);
	while(!isempty())
...
void Records :: mergeSort(int left, int right){
    if (left >= right)
        return;
...
int main(){
    ...
    while(1){
        ...
        switch(ch){
            ...
            case 2:
                    r.rec_quick(0, n - 1);
                    cout<<"\nRecords are sorted using Recursive Quicksort method"<<endl;
                    r.display();
            ...
            case 3:
                    r.non_rec_quick(0, n - 1);
                    cout<<"\nRecords are sorted using Non-Recursive Quicksort method"<<endl;
                    r.display();
            ...
            case 4:
                    r.mergeSort(0,n-1);
                    cout<<"\nRecords are sorted using Mergsort method"<<endl;
                    r.display();               
                    break;
            
            case 5: r.heapsort();
                    cout<<"\nRecords are sorted using Heapsort method"<<endl;
                    r.display();
                    break;
            default:
                    cout<<"\n Invalid choice";
